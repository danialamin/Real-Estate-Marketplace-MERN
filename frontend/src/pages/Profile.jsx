...
const Profile = () => {  
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await fetch(`${window.location.origin}/updateUser/${currentUser._id}`, {
        method: 'POST',
        credentials: 'include',
  ...
  const handleDelete = async () => {
    const res = await fetch(`${window.location.origin}/deleteUser/${currentUser._id}`, {
      method: 'DELETE',
      credentials: 'include'
    })
    const data = await res.json()
  ...
  const handleSignOut = async () => {
    const res = await fetch(`${window.location.origin}/signOut/${currentUser._id}`, {
      method: 'GET',
      credentials: 'include'
    })
    const data = await res.json()
  ...
  const handleShowListings = async () => {
    const res = await fetch(`${window.location.origin}/listing/myListing/${currentUser._id}`, {
      method: 'GET',
      credentials: 'include'
    })
    const data = await res.json()
  ...
  const handleDeleteListing = async (listingId) => {
    const res = await fetch(`${window.location.origin}/listing/deleteMyListing/${currentUser._id}`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      credentials: "include",
